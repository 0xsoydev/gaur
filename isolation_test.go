...
func TestMarkedPackagesIsolationOnModeSwitch(t *testing.T) {
	...
	if len(m.markedPackages) != 0 {
		t.Errorf("markedPackages should be empty after switching to Remove mode, got %v", m.markedPackages)
	}

	// 3. Mark something else in Remove mode (mock installed list)
	m.installed = []Package{{Source: "core", Name: "linux", Version: "6.0"}}
	m.filteredInstalled = m.installed
	m.selectedIndex = 0
	m.handleMarking()
	if !m.markedPackages["linux"] {
		t.Fatal("Expected linux to be marked in Remove mode")
	}
